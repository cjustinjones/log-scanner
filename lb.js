...
const workers = new Map([
    ["/var/log/syslog", { "port": 3001 }],
    ["/var/log/auth.log", { "port": 3002 }],
    ["/var/log/kern.log", { "port": 3003 }],
    ["/windows/logs/DAX2_API_Install.log", { "port": 3004 }]
]);
...
const LaunchWorkers = (workers) => {
    workers.forEach((v, k) => {
        ...
        v.proc = fork(
            "worker.js", [v.port, k], { signal: signal, killSignal: 'SIGINT' }
        )
        v.proc.on("error", (err) => {
            console.log(err);
        })
...
const forwardHandler = async (req, res) => {
